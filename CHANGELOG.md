...
- `IServiceRegistry.BindSelf<T>` and `ServiceLocator.BindSelf<T>` — register a CONCRETE type under its
  own type key, skipping the interface-only constraint. Enables service-locator style registration of
  concrete classes (e.g. migrating an existing static registry onto the container).
